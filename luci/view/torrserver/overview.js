...
/* CPU-дельта между двумя вызовами /proc/stat */
let _prevStat = null;

function getCpuStat(pid) {
...
function getProcCpu(pid) {
    ...
    return Promise.all([
        ...
        const pstat  = (results[0] || '').split(/\s+/);
        const slines = (results[1] || '').split('\n');
        ...
        if (!_prevProcStat) { _prevProcStat = { proc: procTotal, sys: sysTotal }; return '0'; }
...
return view.extend({
    ...
    render: function(data) {
        ...
        const coresTxt = E('div', { id: 'ts-cores-txt', class: 'ts-sub' }, '0%|0%|0%|0%');
        wrap.appendChild(E('div', { class: 'ts-card' }, [
        ...
        /* ── CSS ── */
        root.insertBefore(this._renderStyles(), root.firstChild);
        ...
        function schedulePoll() {
            _pollTimer = setTimeout(function() { tick(); schedulePoll(); }, delay());
        }
        tick();
        schedulePoll();
    ...
    _renderSettings: function() {
        ...
        function inp(opt, placeholder, type) {
            ...
            E('button', {
                ...
                click: function() {
                    ...
                    uci.load('torrserver').then(function() {
                        ...
                        if (r) r.replaceWith(this._renderSettings());
                    });
                }
            }, 'Сбросить')
        ]));
